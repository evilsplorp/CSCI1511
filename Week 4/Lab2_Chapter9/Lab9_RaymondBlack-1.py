...
def main():
    ...
    while True:
        ...
        if play == "n":
            ...
            valid_or_reason = player1.is_valid_name()

            while valid_or_reason != True:
                player1 = Player(input(f"{valid_or_reason} Please enter a name, Player 1: "))
                valid_or_reason = player1.is_valid_name()

            player2 = Player(input("Second player? "))
            valid_or_reason = player2.is_valid_name(player1)

            while valid_or_reason != True:
                player2 = Player(input(f"{valid_or_reason} Please enter a name, Player 2: "))
                valid_or_reason = player2.is_valid_name(player1)
            ...
            while play == "y":
                ...
                play = input("\nDo you want to toss another coin? (y/n)) ").strip().lower()
                if play == "y":
                    continue
                elif play == "n":
                    break
